...
class ChatMessageViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            listing = Listing.objects.filter(id=listing_pk).first()
            if listing is None:
                raise Listing.DoesNotExist(f"Listing with ID {listing_pk} not found")
            serializer.save(user=self.request.user, listing=listing)
        except Exception as e:
            # Add more context to the error
            raise Exception(f"Error creating message: {str(e)}. Listing ID: {listing_pk}, User: {self.request.user.username}")
    
    @action(detail=True, methods=['post'], permission_classes=[permissions.IsAuthenticated])
    def mark_read(self, request, pk=None, listing_pk=None):
